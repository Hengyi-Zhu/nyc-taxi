'''
In the original green and yellow files before year 2015, there is a carriage return in the first row. 
Remove those, otherwise there is error when trying to copy to PostgreSQL. 
'''
...
urls_green = [
"https://storage.googleapis.com/tlc-trip-data/2013/green_tripdata_2013-08.csv",
...
"https://storage.googleapis.com/tlc-trip-data/2015/yellow_tripdata_2015-12.csv"]


def main():
    for i in range(len(urls_green)):
        urllib.urlretrieve(urls_green[i], folder + urls_green[i].split("/")[-1])
    for i in range(len(urls_yellow)):
        urllib.urlretrieve(urls_yellow[i], folder + urls_yellow[i].split("/")[-1])
